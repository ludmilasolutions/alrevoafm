...
// Clave para persistencia del carrito por usuario
const STORAGE_KEY_PREFIX = 'pos_carrito_';
...
document.getElementById('logout-btn').addEventListener('click', async function() {
    try {
        ...
        limpiarEstadoVentaPersistido();

        // Conservar el modo oscuro en el nuevo estado
        const modoOscuroActual = appState.modoOscuro;
        appState = {
            usuario: null,
            permisos: [],
            carrito: [],
...
async function hasPermission(permiso) {
    if (appState.usuario?.rol === 'Administrador') return true;
...
function guardarEstadoVenta() {
    ...
    const key = STORAGE_KEY_PREFIX + appState.usuario.id;
    const estado = {
...
function cargarEstadoVenta() {
    ...
    if (saved) {
        try {
            ...
            // Restaurar valores en los inputs de descuento
            if (appState.descuento.valor > 0) {
                ...
                if (descuentoTipo) descuentoTipo.value = appState.descuento.tipo;
            }
        } catch (e) {
...
function limpiarEstadoVentaPersistido() {
    if (!appState.usuario) return;
    const key = STORAGE_KEY_PREFIX + appState.usuario.id;
...
function setupEventListeners() {
    // Botón de tema (centralizado aquí, no en HTML)
    ...
    document.getElementById('btn-refrescar-reportes')?.addEventListener('click', generarReporte);
    document.getElementById('btn-imprimir-reporte')?.addEventListener('click', imprimirReporte);
...
function showSection(sectionId) {
    ...
    switch(sectionId) {
        ...
        case 'historial': cargarVentasHoy(); break;
        case 'caja': cargarEstadoCaja(); break;
        case 'reportes':
            inicializarFechasReportes();
            setTimeout(() => document.getElementById('btn-generar-reporte')?.click(), 100);
            break;
        case 'configuracion': cargarConfiguracionTicket(); break;
    }
}
    ...
    document.addEventListener('keydown', function(e) {
        ...
        switch(e.key) {
            case 'F1':
                ...
                if (btnFinalizar && !btnFinalizar.disabled) finalizarVenta();
                break;
            case 'F2':
                e.preventDefault();
...
async function buscarYAgregarProducto(codigo) {
    ...
    try {
        const { data: producto, error } = await supabaseClient
            ...
            .single();
        if (error) {
            if (error.code === 'PGRST116') showNotification('Producto no encontrado', 'warning');
            else throw error;
            return;
        }
        if (producto) {
            const index = appState.carrito.findIndex(item => item.producto.id === producto.id);
            if (index !== -1) {
            ...
            if (typeof playScanSound === 'function') playScanSound();
        }
    } catch (error) {
...
function actualizarCarritoUI() {
    ...
    const btnFinalizar = document.getElementById('btn-finalizar-venta');
    if (!container || !subtotalEl || !descuentoEl || !totalEl || !totalAPagarEl || !totalFinalEl || !btnFinalizar) return;
...
function seleccionarMedioPago(medio) {
    document.querySelectorAll('.btn-pago').forEach(btn => btn.classList.remove('active'));
...
window.eliminarPagosPorMedio = function(medio) {
    if (!confirm(`¿Eliminar todos los pagos de ${medio}?`)) return;
...
function actualizarPagosUI() {
    ...
    appState.pagos.forEach(pago => {
        if (!pagosAgrupados[pago.medio]) pagosAgrupados[pago.medio] = 0;
    ...
    const hayExceso = totalPagado > totalAPagar && totalAPagar > 0;
    ...
    Object.entries(pagosAgrupados).forEach(([medio, monto]) => {
        ...
        div.innerHTML = `
            ...
            <div class="pago-monto">$ ${monto.toFixed(2)}</div>
    ...
    // Advertencia y botón de ajuste si hay exceso
    if (hayExceso) {
        const ajusteDiv = document.createElement('div');
        ajusteDiv.className = 'pago-ajuste';
        ajusteDiv.innerHTML = `
            <div class="text-warning small mt-2">
                <i class="fas fa-exclamation-triangle"></i> 
                Los pagos exceden el total en $ ${(totalPagado - totalAPagar).toFixed(2)}
            </div>
            <button id="btn-ajustar-pagos" class="btn btn-sm btn-warning mt-1">
                <i class="fas fa-adjust"></i> Ajustar pagos al total
            </button>
        `;
        container.appendChild(ajusteDiv);
        document.getElementById('btn-ajustar-pagos')?.addEventListener('click', ajustarPagosAlTotal);
    }
    ...
    // Deshabilitar botón finalizar si hay exceso de pago
    if (hayExceso) {
        btnFinalizar.disabled = true;
        btnFinalizar.title = 'Debe ajustar los pagos antes de finalizar';
    } else {
        btnFinalizar.disabled = !(appState.carrito.length > 0 && totalPagado >= totalAPagar);
        btnFinalizar.title = 'Finalizar venta (F1)';
    }
    ...
    if (!hayExceso && totalPagado >= totalAPagar && appState.carrito.length > 0) {
        setTimeout(() => btnFinalizar.focus(), 100);
    }
}

function ajustarPagosAlTotal() {
    const totalAPagar = parseFloat(document.getElementById('carrito-total').textContent.replace('$ ', ''));
    let totalPagado = appState.pagos.reduce((sum, p) => sum + p.monto, 0);
    if (totalPagado <= totalAPagar) return;
    const exceso = totalPagado - totalAPagar;
    if (appState.pagos.length > 0) {
        const ultimoPago = appState.pagos[appState.pagos.length - 1];
        if (ultimoPago.monto > exceso) {
            ultimoPago.monto -= exceso;
        } else {
            let sobrante = exceso;
            while (sobrante > 0 && appState.pagos.length > 0) {
                const pago = appState.pagos[appState.pagos.length - 1];
                if (pago.monto <= sobrante) {
                    sobrante -= pago.monto;
                    appState.pagos.pop();
                } else {
                    pago.monto -= sobrante;
    ...
    actualizarPagosUI();
    guardarEstadoVenta();
    showNotification('Pagos ajustados al total de la venta', 'success');
}
...
async function finalizarVenta() {
    ...
    try {
        ...
        const subtotal = appState.carrito.reduce((sum, item) => sum + (item.cantidad * item.precioUnitario), 0);
        const descuento = appState.descuento.valor > 0 ? 
            (appState.descuento.tipo === 'porcentaje' ? subtotal * (appState.descuento.valor / 100) : appState.descuento.valor) : 0;
        const total = subtotal - descuento;
...
async function requestPrinter() {
    if (appState.usbPrinterDevice) return appState.usbPrinterDevice;
    if (!navigator.usb) {
...
async function printESC/POSTicket(venta, configMap, carrito, pagos, usuario, cambio) {
    ...
    const addBytes = (...bytes) => appState.escposBuffer.push(...bytes);
    const addText = (text) => {
        if (!text) return;
    ...
    carrito.forEach(item => {
        const nombre = item.producto.nombre.length > 20 ? item.producto.nombre.substring(0, 20) + '.' : item.producto.nombre.padEnd(20);
    ...
    addText('');
    addText('PAGOS:');
    pagos.forEach(pago => addText(`${pago.medio}: $${pago.monto.toFixed(2)}`));
    if (cambio > 0) addText(`Cambio: $${cambio.toFixed(2)}`);
    separator();
    ...
    if (success) showNotification('Ticket impreso correctamente', 'success');
    return success;
}
...
function printViaHTML(venta, configMap, carrito, pagos, usuario, cambio) {
    ...
    carrito.forEach(item => {
        ...
        const nombre = item.producto.nombre.length > 20 ? item.producto.nombre.substring(0, 20) + '...' : item.producto.nombre;
        itemsHTML += `
...
async function generarTicket(venta) {
    try {
        ...
        const escposSuccess = await printESC/POSTicket(venta, configMap, appState.carrito, appState.pagos, appState.usuario, cambio);
        if (!escposSuccess) {
...
async function verificarCajaActiva() {
    try {
        const { data: caja, error } = await supabaseClient
            ...
            .maybeSingle();
        if (error && error.code !== 'PGRST116') console.warn('Error verificando caja activa:', error);
        appState.cajaActiva = caja || null;
        actualizarUIEstadoCaja();
        if (caja) await cargarVentasDelDiaEnCaja();
        return caja;
    } catch (error) {
...
async function cargarEstadoCaja() {
    await verificarCajaActiva();
    if (appState.cajaActiva) await cargarVentasDelDiaEnCaja();
}
...
function actualizarUIEstadoCaja() {
    ...
    if (appState.cajaActiva) {
        ...
        if (btn) btn.onclick = mostrarModalAperturaCaja;
    }
}
...
async function abrirCaja(e) {
    ...
    try {
        ...
        const { data: caja, error } = await supabaseClient
            ...
            .single();
        if (error) {
            if (error.code === '23505') showNotification('Ya hay una caja activa', 'error');
            else throw error;
        } else {
...
async function cargarResumenCaja() {
    ...
    try {
        ...
        ventas.forEach(venta => {
            if (venta.pagos_venta && venta.pagos_venta.length > 0) {
                venta.pagos_venta.forEach(pago => {
                    if (pago.medio_pago === 'EFECTIVO') totalEfectivo += parseFloat(pago.monto);
                    else if (pago.medio_pago === 'TARJETA') totalTarjeta += parseFloat(pago.monto);
                    else if (pago.medio_pago === 'TRANSFERENCIA/QR') totalTransferencia += parseFloat(pago.monto);
                });
            }
        });
        ...
        if (montoInicialEl) montoInicialEl.textContent = `$ ${parseFloat(appState.cajaActiva.monto_inicial).toFixed(2)}`;
        if (ventasEfectivoEl) ventasEfectivoEl.textContent = `$ ${totalEfectivo.toFixed(2)}`;
        if (ventasTarjetaEl) ventasTarjetaEl.textContent = `$ ${totalTarjeta.toFixed(2)}`;
        if (ventasTransferenciaEl) ventasTransferenciaEl.textContent = `$ ${totalTransferencia.toFixed(2)}`;

        const totalEstimado = parseFloat(appState.cajaActiva.monto_inicial) + totalEfectivo + totalTarjeta + totalTransferencia;
        if (totalEstimadoEl) totalEstimadoEl.textContent = `$ ${totalEstimado.toFixed(2)}`;
    ...
    try {
        ...
        if (userIds.length > 0) {
            ...
            if (!usuariosError && usuarios) {
                usuarios.forEach(u => usuariosMap[u.id] = u.username);
            }
        }
...
async function cargarDetallesCajaDia() {
    try {
        ...
        if (ventas && ventas.length > 0) {
            ...
            let totalVentas = 0, ventasEfectivo = 0, ventasTarjeta = 0, ventasTransferencia = 0;
            const pagosPorVenta = {};
            if (pagos && !pagosError) {
                pagos.forEach(pago => {
            ...
            ventas.forEach(venta => {
                ...
                pagosVenta.forEach(pago => {
                    ...
                    if (pago.medio_pago === 'EFECTIVO') ventasEfectivo += monto;
                    else if (pago.medio_pago === 'TARJETA') ventasTarjeta += monto;
                    else if (pago.medio_pago === 'TRANSFERENCIA/QR') ventasTransferencia += monto;
                });
            });
            ...
            if (userIds.length > 0) {
                const { data: usuarios } = await supabaseClient
                    ...
                    .select('id, username')
                    .in('id', userIds);
                if (usuarios) usuarios.forEach(u => usuariosMap[u.id] = u.username);
            }
                ...
                html += `
                    <div class="list-item">
                        ...
                        <div class="text-muted small">
                            ${new Date(venta.fecha).toLocaleTimeString('es-ES')} - ${nombreUsuario}
                        </div>
                    </div>
                `;
            }
...
async function cargarVentasDelDiaEnCaja() {
    ...
    try {
        ...
        if (ventas && ventas.length > 0) {
            ventas.forEach(venta => totalVentas += parseFloat(venta.total) || 0);
        }
    ...
    try {
        ...
        if (ventas.length > 0) {
            ...
            ventas.forEach(venta => {
                ...
                if (venta.pagos_venta && venta.pagos_venta.length > 0) {
                    venta.pagos_venta.forEach(pago => {
                        if (!medios[pago.medio_pago]) medios[pago.medio_pago] = 0;
...
window.forzarCerrarCaja = async function(cajaId) {
    if (!confirm('¿Está seguro de forzar el cierre de esta caja? Esta acción no se puede deshacer.')) return;
    try {
...
async function cargarHistorial() {
    try {
        ...
        const { data: pagos, error: pagosError } = await supabaseClient
            ...
            .select('*')
            .in('venta_id', ventaIds);
        if (pagosError) console.error('Error obteniendo pagos:', pagosError);
        ...
        if (userIds.length > 0) {
            const { data: usuarios, error: usuariosError } = await supabaseClient
                ...
                .select('id, username')
                .in('id', userIds);
            if (!usuariosError && usuarios) usuarios.forEach(u => usuariosMap[u.id] = u.username);
        }
...
window.anularVenta = async function(id) {
    ...
    if (!confirm('¿Está seguro de anular esta venta? Esta acción revertirá el stock y no se podrá deshacer.')) return;
    try {
        const { error } = await supabaseClient
            .from('ventas')
            .update({
...
async function cargarProductos() {
    try {
        ...
        if (estado && estado.value) {
            if (estado.value === 'activos') query = query.eq('activo', true);
            else if (estado.value === 'inactivos') query = query.eq('activo', false);
        }
        ...
        productos.forEach(producto => {
            const tr = document.createElement('tr');
            const margen = producto.margen_ganancia ? `${parseFloat(producto.margen_ganancia).toFixed(2)}%` : 'N/A';
            tr.innerHTML = `
...
async function guardarProducto(e) {
    ...
    try {
        ...
        if (error) {
            if (error.code === '23505') throw new Error('El código de barras ya existe');
            throw error;
        }
...
window.eliminarProducto = async function(id) {
    ...
    if (!confirm('¿Está seguro de eliminar este producto? Se marcará como inactivo.')) return;
    try {
        const { error } = await supabaseClient
            .from('productos')
            .update({ activo: false })
...
async function generarReporte() {
    try {
        ...
        let reporteHTML = '';
        switch(tipoReporte) {
            case 'general': reporteHTML = await generarReporteGeneral(ventas, productos, cajas, fechaInicio, fechaFin); break;
            case 'ventas': reporteHTML = generarReporteVentas(ventas, fechaInicio, fechaFin); break;
            case 'productos': reporteHTML = generarReporteProductos(ventas, productos, fechaInicio, fechaFin); break;
            case 'ganancias': reporteHTML = generarReporteGanancias(ventas, fechaInicio, fechaFin); break;
            default: reporteHTML = await generarReporteGeneral(ventas, productos, cajas, fechaInicio, fechaFin);
        }

        const resultados = document.getElementById('reportes-resultados');
        if (resultados) resultados.innerHTML = reporteHTML;
...
async function generarReporteGeneral(ventas, productos, cajas, fechaInicio, fechaFin) {
    let totalVentas = 0, totalProductosVendidos = 0, totalGanancias = 0, totalDescuentos = 0;
    const ventasPorDia = {}, productosVendidos = {}, mediosPago = { 'EFECTIVO': 0, 'TARJETA': 0, 'TRANSFERENCIA/QR': 0 };

    ventas?.forEach(venta => {
        totalVentas += parseFloat(venta.total) || 0;
        totalDescuentos += parseFloat(venta.descuento) || 0;
        const fecha = venta.fecha.split('T')[0];
        if (!ventasPorDia[fecha]) ventasPorDia[fecha] = { ventas: 0, cantidad: 0 };
        ...
        if (venta.detalle_ventas) {
            venta.detalle_ventas.forEach(detalle => {
                ...
                if (!productosVendidos[productoNombre]) productosVendidos[productoNombre] = { cantidad: 0, total: 0 };
        ...
        if (venta.pagos_venta) {
            venta.pagos_venta.forEach(pago => {
                if (mediosPago[pago.medio_pago] !== undefined) mediosPago[pago.medio_pago] += parseFloat(pago.monto) || 0;
            });
        }
    });
    ...
    return `
        ...
        <div class="row mt-4">
            <div class="col-md-6">
                <div class="reporte-desglose">
                    ...
                    <table class="data-table">
                        <thead><tr><th>Medio de Pago</th><th>Total</th><th>Porcentaje</th></tr></thead>
                        <tbody>
                            ${Object.entries(mediosPago).map(([medio, total]) => {
                                const porcentaje = totalVentas > 0 ? (total / totalVentas * 100) : 0;
                                return `<tr><td>${medio}</td><td>$ ${total.toFixed(2)}</td><td>${porcentaje.toFixed(1)}%</td></tr>`;
                            }).join('')}
                        </tbody>
                    </table>
                </div>
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                ${productosTop.map(([nombre, datos]) => `
                                    <tr><td>${nombre}</td><td>${datos.cantidad}</td><td>$ ${datos.total.toFixed(2)}</td></tr>
                                `).join('')}
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="mt-4">
            ...
            <div style="max-height: 300px; overflow-y: auto;">
                <table class="data-table">
                    <thead><tr><th>Fecha</th><th>Ventas</th><th>Total</th><th>Promedio</th></tr></thead>
                    <tbody>
                        ${diasOrdenados.map(([fecha, datos]) => `
                            <tr><td>${fecha}</td><td>${datos.cantidad}</td><td>$ ${datos.ventas.toFixed(2)}</td><td>$ ${(datos.ventas / datos.cantidad).toFixed(2)}</td></tr>
                        `).join('')}
                    </tbody>
                </table>
            </div>
        ...
        <div class="mt-4 p-3 bg-light rounded">
            ...
            <div class="row">
                <div class="col-md-4"><p><strong>Total Descuentos:</strong> $ ${totalDescuentos.toFixed(2)}</p><p><strong>Total Cajas:</strong> ${cajas?.length || 0}</p></div>
                <div class="col-md-4"><p><strong>Días con Ventas:</strong> ${diasOrdenados.length}</p><p><strong>Productos Diferentes:</strong> ${Object.keys(productosVendidos).length}</p></div>
                <div class="col-md-4"><p><strong>Ventas/Día Promedio:</strong> $ ${diasOrdenados.length > 0 ? (totalVentas / diasOrdenados.length).toFixed(2) : '0'}</p><p><strong>Productos/Venta Promedio:</strong> ${ventas?.length > 0 ? (totalProductosVendidos / ventas.length).toFixed(1) : '0'}</p></div>
            </div>
        </div>
    `;
}
...
function generarReporteVentas(ventas, fechaInicio, fechaFin) {
    ...
    let html = `
        ...
        <div class="table-responsive">
            <table class="data-table">
                <thead>
                    <tr><th>Ticket</th><th>Fecha</th><th>Subtotal</th><th>Descuento</th><th>Total</th><th>Medios de Pago</th><th>Acciones</th></tr>
                </thead>
                <tbody>
    `;
    ventas.forEach(venta => {
        ...
        if (venta.pagos_venta && venta.pagos_venta.length > 0) {
            venta.pagos_venta.forEach(pago => {
                if (!mediosPago[pago.medio_pago]) mediosPago[pago.medio_pago] = 0;
                mediosPago[pago.medio_pago] += parseFloat(pago.monto);
            });
        }
        const mediosTexto = Object.entries(mediosPago).map(([medio, monto]) => `${medio}: $ ${monto.toFixed(2)}`).join(', ');
        html += `
            <tr>
                <td>${venta.ticket_id}</td>
                <td>${fecha}</td>
                ...
                <td>$ ${parseFloat(venta.total).toFixed(2)}</td>
                <td>${mediosTexto || 'Sin pagos registrados'}</td>
                <td><button class="btn btn-sm" onclick="verDetalleVenta('${venta.id}')"><i class="fas fa-eye"></i></button></td>
            </tr>
        `;
    });
...
function generarReporteProductos(ventas, productos, fechaInicio, fechaFin) {
        ...
        if (venta.detalle_ventas) {
            venta.detalle_ventas.forEach(detalle => {
                ...
                if (!productosVendidos[productoNombre]) {
                    productosVendidos[productoNombre] = { cantidad: 0, total: 0, costo: 0, ganancia: 0 };
                }
                productosVendidos[productoNombre].cantidad += detalle.cantidad || 0;
                productosVendidos[productoNombre].total += parseFloat(detalle.subtotal) || 0;
                ...
                productosVendidos[productoNombre].ganancia = productosVendidos[productoNombre].total - productosVendidos[productoNombre].costo;
            });
        }
    });
    ...
    return `
        ...
        <div class="reporte-resumen">
            <div class="reporte-item"><h4>PRODUCTOS VENDIDOS</h4><div class="reporte-valor">${productosArray.length}</div><small>Productos diferentes</small></div>
            <div class="reporte-item"><h4>UNIDADES TOTALES</h4><div class="reporte-valor">${productosArray.reduce((sum, p) => sum + p.cantidad, 0)}</div><small>Unidades vendidas</small></div>
            <div class="reporte-item"><h4>TOTAL VENTAS</h4><div class="reporte-valor">$ ${productosArray.reduce((sum, p) => sum + p.total, 0).toFixed(2)}</div><small>En productos</small></div>
            <div class="reporte-item"><h4>GANANCIAS</h4><div class="reporte-valor">$ ${productosArray.reduce((sum, p) => sum + p.ganancia, 0).toFixed(2)}</div><small>Por productos</small></div>
        </div>
        <div class="mt-4">
            <h4><i class="fas fa-list-ol"></i> Detalle de Productos Vendidos</h4>
            <div style="max-height: 400px; overflow-y: auto;">
                <table class="data-table">
                    <thead><tr><th>Producto</th><th>Cantidad</th><th>Ventas</th><th>Costo</th><th>Ganancia</th><th>Margen %</th></tr></thead>
                    <tbody>
                        ${productosArray.map(producto => {
                            const margen = producto.costo > 0 ? (producto.ganancia / producto.costo * 100) : 0;
                            return `<tr><td>${producto.nombre}</td><td>${producto.cantidad}</td><td>$ ${producto.total.toFixed(2)}</td><td>$ ${producto.costo.toFixed(2)}</td><td>$ ${producto.ganancia.toFixed(2)}</td><td>${margen.toFixed(1)}%</td></tr>`;
                        }).join('')}
                    </tbody>
                </table>
            </div>
...
function generarReporteGanancias(ventas, fechaInicio, fechaFin) {
    let totalVentas = 0, totalCosto = 0, totalGanancias = 0, totalDescuentos = 0;
    const gananciasPorDia = {};
    ventas?.forEach(venta => {
        totalVentas += parseFloat(venta.total) || 0;
        totalDescuentos += parseFloat(venta.descuento) || 0;
        const fecha = venta.fecha.split('T')[0];
        if (!gananciasPorDia[fecha]) gananciasPorDia[fecha] = { ventas: 0, costo: 0, ganancia: 0 };
        if (venta.detalle_ventas) {
            let ventaCosto = 0, ventaGanancia = 0;
            venta.detalle_ventas.forEach(detalle => {
                const costo = parseFloat(detalle.productos?.precio_costo) || 0;
    ...
    const diasOrdenados = Object.entries(gananciasPorDia).sort((a, b) => new Date(a[0]) - new Date(b[0]));
    ...
    return `
        ...
        <div class="reporte-resumen">
            <div class="reporte-item"><h4>VENTAS TOTALES</h4><div class="reporte-valor">$ ${totalVentas.toFixed(2)}</div><small>Ingresos brutos</small></div>
            <div class="reporte-item"><h4>COSTOS TOTALES</h4><div class="reporte-valor">$ ${totalCosto.toFixed(2)}</div><small>Inversión en productos</small></div>
            <div class="reporte-item"><h4>GANANCIAS NETAS</h4><div class="reporte-valor">$ ${totalGanancias.toFixed(2)}</div><small>Utilidad real</small></div>
            <div class="reporte-item"><h4>MARGEN %</h4><div class="reporte-valor">${margenTotal.toFixed(1)}%</div><small>Rentabilidad</small></div>
        </div>
        <div class="mt-4">
            <h4><i class="fas fa-chart-line"></i> Evolución Diaria de Ganancias</h4>
            <div style="max-height: 300px; overflow-y: auto;">
                <table class="data-table">
                    <thead><tr><th>Fecha</th><th>Ventas</th><th>Costos</th><th>Ganancias</th><th>Margen %</th></tr></thead>
                    <tbody>
                        ${diasOrdenados.map(([fecha, datos]) => {
                            const margenDia = datos.costo > 0 ? (datos.ganancia / datos.costo * 100) : 0;
                            return `<tr><td>${fecha}</td><td>$ ${datos.ventas.toFixed(2)}</td><td>$ ${datos.costo.toFixed(2)}</td><td>$ ${datos.ganancia.toFixed(2)}</td><td>${margenDia.toFixed(1)}%</td></tr>`;
                        }).join('')}
                    </tbody>
                </table>
            </div>
        ...
        <div class="mt-4 p-3 bg-light rounded">
            ...
            <div class="row">
                <div class="col-md-6"><p><strong>Descuentos Aplicados:</strong> $ ${totalDescuentos.toFixed(2)}</p><p><strong>Ventas Netas:</strong> $ ${(totalVentas + totalDescuentos).toFixed(2)}</p></div>
                <div class="col-md-6"><p><strong>Ganancia/Venta Promedio:</strong> $ ${ventas?.length > 0 ? (totalGanancias / ventas.length).toFixed(2) : '0'}</p><p><strong>Rentabilidad Diaria Promedio:</strong> $ ${diasOrdenados.length > 0 ? (totalGanancias / diasOrdenados.length).toFixed(2) : '0'}</p></div>
            </div>
        </div>
    `;
}
...
function mostrarTabConfiguracion(tabId) {
    ...
    if (tabId === 'config-permisos') cargarConfiguracionPermisos();
    else if (tabId === 'config-usuarios') cargarConfiguracionUsuarios();
}
...
async function cargarConfiguracionPermisos() {
    try {
        ...
        for (const usuario of usuarios) {
            ...
            const permisosActivos = permisosUsuario.filter(p => p.activo).map(p => p.permiso);
                ...
                <div class="usuario-permisos">
                    ...
                    <div class="permisos-lista">
                        <div class="permiso-item">
                            <label><input type="checkbox" data-usuario="${usuario.id}" data-permiso="cargar_productos" ${permisosActivos.includes('cargar_productos') ? 'checked' : ''}> Cargar Productos</label>
                        </div>
                        <div class="permiso-item">
                            <label><input type="checkbox" data-usuario="${usuario.id}" data-permiso="modificar_productos" ${permisosActivos.includes('modificar_productos') ? 'checked' : ''}> Modificar Productos</label>
                        </div>
                        <div class="permiso-item">
                            <label><input type="checkbox" data-usuario="${usuario.id}" data-permiso="anular_ventas" ${permisosActivos.includes('anular_ventas') ? 'checked' : ''}> Anular Ventas</label>
                        </div>
                        <div class="permiso-item">
                            <label><input type="checkbox" data-usuario="${usuario.id}" data-permiso="ver_reportes" ${permisosActivos.includes('ver_reportes') ? 'checked' : ''}> Ver Reportes</label>
                        </div>
                        <div class="permiso-item">
                            <label><input type="checkbox" data-usuario="${usuario.id}" data-permiso="acceder_caja" ${permisosActivos.includes('acceder_caja') ? 'checked' : ''}> Acceder a Caja</label>
                        </div>
                    </div>
...
async function cargarConfiguracionUsuarios() {
    if (appState.usuario?.rol !== 'Administrador') {
        ...
        if (usuariosList) {
            usuariosList.innerHTML = `<p class="text-warning">Solo administradores pueden acceder a esta sección</p>`;
        }
        return;
    }
    try {
        ...
        let html = `
            <table class="data-table">
                <thead><tr><th>Usuario</th><th>Rol</th><th>Estado</th><th>Fecha Creación</th></tr></thead>
                <tbody>
        `;
        usuarios.forEach(usuario => {
            html += `
                <tr>
                    ...
                    <td>${usuario.rol}</td>
                    <td><span class="${usuario.activo ? 'text-success' : 'text-danger'}">${usuario.activo ? 'Activo' : 'Inactivo'}</span></td>
                    <td>${new Date(usuario.creado_en).toLocaleDateString('es-ES')}</td>
                </tr>
            `;
        });
        html += `</tbody></table><p class="mt-3"><small>Nota: Para agregar nuevos usuarios, utilice la consola de autenticación de Supabase.</small></p>`;
        usuariosList.innerHTML = html;
    } catch (error) {
        console.error('Error cargando usuarios:', error);
        const usuariosList = document.getElementById('config-usuarios-list');
        if (usuariosList) {
            usuariosList.innerHTML = `<p>Error cargando usuarios: ${error.message}</p>`;
        }
    }
}
...
function showNotification(mensaje, tipo = 'info') {
    ...
    notification.innerHTML = `<i class="fas fa-${icon}"></i><span>${mensaje}</span>`;
